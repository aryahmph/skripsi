...
func RunJobCategory(ctx context.Context) {
	...
	ucase := job.NewUpdateTicketCategoryCache(cfg, ticketRepo, cacher, dqp)

	dqWatcher.Watch(&dq.WatcherContext{
		Handler:  handler.DQWatcherHandler(ucase),
		Context:  ctx,
		Name:     cfg.Job.TicketCategory.QueueName,
		Interval: time.Duration(cfg.Job.TicketCategory.IntervalSecond) * time.Second,
		Limit:    cfg.Job.TicketCategory.DataPerInterval,
	})
}
