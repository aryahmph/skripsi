...
import (
	...
	"en-payment/internal/ucase/v1/payment"
	"en-payment/pkg/postgres"
	"encoding/json"
	"net/http"
	"runtime/debug"
	"time"
	...
	"en-payment/internal/bootstrap"
	"en-payment/internal/consts"
	"en-payment/internal/handler"
	"en-payment/internal/middleware"
	repository "en-payment/internal/repositories"
	"en-payment/internal/ucase"
	ucaseContract "en-payment/internal/ucase/contract"
...
func (rtr *router) Route() *routerkit.Router {
	...
	orderProvider := order.NewOrderProvider(rtr.config)

	createPayment := payment.NewCreatePayment(kp, paymentRepo, orderProvider)
	internalGetPayment := payment.NewInternalGetPayment(cacher, paymentRepo)
	...
	inV1.HandleFunc("/payments/{id}", rtr.handle(
		handler.HttpRequest,
		internalGetPayment,
