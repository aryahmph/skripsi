...
func TestSliceStructToBulkInsert(t *testing.T) {
	...
	for _, x := range testCase {
		c, v, r, e := SliceStructToBulkInsert(x.Input, "store")
		if fmt.Sprintf("%T", x.Error) == fmt.Sprintf("%T", e) {
		...
		if len(r) == len(x.Input) {
			t.Logf("expected %v, but got %v", len(x.Input), len(r))
		} else {
			t.Errorf("expected %v, but got %v", len(x.Input), len(r))
		}
