...
func (ucase *assignTicketOrder) Serve(ctx context.Context, data *appctx.ConsumerData) error {
	...
	* ----------------------------------------*/
	...
	ticket, err := ucase.ticketRepo.FindOneTicket(ctx, repositories.FindOneTicketCriteria{ID: respAPI.Data.TicketID})
	if err != nil {
		tracer.SpanError(ctx, err)
	...
	* ----------------------------------------*/
	...
	err = ucase.ticketRepo.UpdateTicket(ctx, entity.Ticket{ID: ticket.ID, OrderID: msgData.Payload.ID})
	if err != nil {
		tracer.SpanError(ctx, err)
