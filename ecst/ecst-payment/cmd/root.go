...
import (
	"context"
	"ecst-payment/cmd/consumer"
	"ecst-payment/cmd/migration"
	"log"
	"os"
	"os/signal"
	"syscall"

	"ecst-payment/cmd/http"
	...
	cmd := []*cobra.Command{
		migrateCmd,
		{
			...
			PreRun: func(cmd *cobra.Command, args []string) {
				go func() {
					consumer.RunConsumerCreateOrder(ctx)
				}()
		...
		{
			Use:   "consumer:order-expire",
			Short: "Run Consumer Order Expire",
			Run: func(cmd *cobra.Command, args []string) {
				consumer.RunConsumerOrderExpire(ctx)
			},
		},
