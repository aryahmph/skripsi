...
type assignTicketOrder struct {
	cfg        *appctx.Config
	kp         kafka.Producer
	ticketRepo repositories.TicketRepository
}

func NewAssignTicketOrder(
	cfg *appctx.Config,
	kp kafka.Producer,
	ticketRepo repositories.TicketRepository,
) ucaseContract.MessageProcessor {
	return &assignTicketOrder{
		cfg:        cfg,
		kp:         kp,
...
func (ucase *assignTicketOrder) Serve(ctx context.Context, data *appctx.ConsumerData) error {
	defer data.Commit()
	...
	var (
		...
		lf = []logger.Field{
			logger.EventName(consts.LogEventNameAssignTicketOrder),
		}
	...
	* ----------------------------------------*/
	...
	ticket, err := ucase.ticketRepo.FindOneTicket(ctx, repositories.FindOneTicketCriteria{ID: msgData.Payload.TicketID})
	if err != nil {
		tracer.SpanError(ctx, err)
	...
	* ----------------------------------------*/
	...
	updateTicket := entity.Ticket{ID: ticket.ID, OrderID: msgData.Payload.ID, Version: ticket.Version + 1}
	err = ucase.ticketRepo.UpdateTicket(ctx, updateTicket)
	if err != nil {
		tracer.SpanError(ctx, err)
	...
	* ----------------------------------------*/
	...
	logger.InfoWithContext(ctx, logger.SetMessageFormat(consts.LogMessageSuccess, consts.LogEventNameAssignTicketOrder), lf...)
	return nil
}
